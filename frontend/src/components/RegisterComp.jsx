...
export default function RegisterComp(){
    ...
    useEffect(()=>{
        memberApi.get("/subscription/goals")
            .then(resp => {
                setgoals(resp.data)
                if(resp.data.length > 0){
                    // FIX: use gid from new database
    ...
    const handleSubmit=(e)=>{
        ...
        authApi.post("/auth/register", {
            name, email, password, phone, age, gender, goal_id
        })
        .then(resp => {
              ...
              <div className="card p-4">
                ...
                <form>
                  ...
                  <div className="mb-3">
                    ...
                    <select className="form-select" value={goal_id} onChange={(e)=>setgoal_id(e.target.value)}>
                      {goals.map(goal => (
                        // FIX: use gid and gname from new database
                        <option key={goal.gid} value={goal.gid}>{goal.gname}</option>
                      ))}
                    </select>
                  </div>
                  {msg && <p className={msg.includes("successful") ? "text-success" : "text-danger"}>{msg}</p>}
                  <button type="submit" className="btn btn-success w-100 mt-2" onClick={handleSubmit}>REGISTER</button>
